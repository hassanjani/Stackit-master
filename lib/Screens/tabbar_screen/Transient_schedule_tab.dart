...
import 'package:flutter_forget_pass_recover_with_verify/Screens/TenantScreen/Tenant_Dropdown/TransientDB/DB_Pilot.dart';
import 'package:flutter_forget_pass_recover_with_verify/Screens/TenantScreen/Tenant_Dropdown/TransientDB/DB_Transient_Air_Registery.dart';
import 'package:flutter_forget_pass_recover_with_verify/Screens/TenantScreen/Tenant_Dropdown/TransientDB/DB_Transient_Aircrafts.dart';
...
class Transient_Schedule_Tab extends StatefulWidget {
  @override
  _Transient_Schedule_TabState createState() => _Transient_Schedule_TabState();
}

// ignore: camel_case_types
class _Transient_Schedule_TabState extends State<Transient_Schedule_Tab> {
  ...
  GetData() async {
    getSharedPref();
    Provider.of<TransientScheduleProvider>(context, listen: false)
        .getServices();
          ...
          child: Column(
            children: [
              ...
              Container(
                  ...
                  child: FutureBuilder(
                    future: Provider.of<TransientScheduleProvider>(context,
                            listen: false)
                        .getServices(),
  ...
  save_Reservation() async {
    var pp = Provider.of<TransientScheduleProvider>(context, listen: false);
    ...
    var map = {
      "id": id,
      "schedule_type": "$_groupValue",
      "tenant_id": "${pp.Tid}",
      "pilot_id": "${pp.Pid}",
      "aircraft_id": "${pp.Aid}",
      "building_id": "${pp.Bid}",
      "is_fuel": fuelstatus ? "1" : "0",
      // "arrival_date": "2022-01-30",
...
class TransientScheduleProvider extends ChangeNotifier {
  String Tid = "";
  String Pid = "";
  String Aid = "";
  String Bid = "";
  ...
  Future getAllBuildings() async {
    ...
    String id = await prefs.getString('userid');
    var response = await http.get(
        "http://wordpresswebsiteprogrammer.com/stackit/public/api/store-aircraft?tenant_id=13",
        headers: {"Accept": "application/json"});
    var jsonBody = response.body;
    var jsonData = json.decode(jsonBody);
    print(response.statusCode);
